...
import {refreshPage, showCopyNotification, showDeleteConfirmationDialog} from "../allservice";
...
export class UsefullLinksComponent implements OnInit{
  ...
  loadData() {
    if (this.keywords === null) {
      ...
      this.http.get<any>(`${apiUrl}/usefull-link/search/${this.keywords}`).subscribe({
        next: value => {
          this.links = value.data
        }
      })
  ...
  search(form: NgForm) {
    ...
    this.router.navigate(['/dashboard'], {skipLocationChange: true}).then(() => {
      this.router.navigate([`/dashboard/usefull-links/${formValue.keyword}`]).then();
    })
  }
