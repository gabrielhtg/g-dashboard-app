...
export class OcrBankPyComponent implements AfterViewInit{
  ...
  tampilkanPesanError() {
    const {element} : any = HSFileUpload.getInstance('#hs-file-upload-with-limited-file-size', true);
  ...
  uploadFile() {
    ...
    this.element.dropzone.on("success", (file: any, response: any) => {
      ...
      // console.log("Response from server:", response); // Dapatkan respons HTTP dari server

      Swal.close()

      this.router.navigate(['/dashboard/ocr-bank-py-processed'], {
        state: response
      }).then();
    });
  }
